...
March 15, 2023 (updated: `r format(Sys.Date(), "%B %d, %Y")`)
...
1. Allows to **re-open** and **re-use** your data in the future.
...
1. You will be forced to buy a license.
...
This is also true for **open source software**.
...
This dataset contains 4 variables (columns). **The first line generally contains the names of the variables.**

.pull-left[
A commas separated values file (`.csv`).
  <figure>
...
These files contain information of geographic features such as **points**, **lines** or **polygons**. There are a ton of [geographical file formats](https://gisgeography.com/gis-formats/), but here are some that are particularly popular.

--

- ESRI shapefile (`.SHP`)

  - Technically, the shapefile format is not open. **It is however widely used and often considered the standard**.
...
Data analysis is an iterative process that can be time-consuming when working with large datasets.
...
- `arrow`: https://arrow.apache.org/
  ...
  - Very fast, optimized for columnar memory format for flat and hierarchical data.
  
  - Available for: R, C, C++, Java, Python, Go, Julia, MATLAB, Ruby, Rust.
...
bm <- microbenchmark::microbenchmark(
  ...
  "arrow::read_csv_arrow" = arrow::read_csv_arrow(file),
  times = 1,
  unit = "milliseconds"
)
```
...
<center>
  <figure>
      <img style="margin:0px auto;display:block" src="http://phdcomics.com/comics/archive/phd052810s.gif" width = "500" alt="Screenshot of a messy file names in a folder."/>
  </figure>
  <figcaption>Image from: https://phdcomics.com/</figcaption>
</center>
...
<center>
    <figure>
      <img style="margin:0px auto;display:block" src="https://www.ncbi.nlm.nih.gov/corecgi/tileshop/tileshop.fcgi?p=PMC3&id=475246&s=95&r=1&c=1" width = "500" alt="Screenshot showing how the same file names are sorted differently on Windows and on Linux."/>
    </figure>
...
.gradient-underline[Always use the ISO format:] <big>**YYYY**</big>-<medium>**MM**</medium>-<small>**DD**</small> (large `r fontawesome::fa("arrow-right", fill = "#d5695d")` small).
  ...
  - For example: 2021-04-21 (21 January 2021)
...
- .gradient-underline[Use a scripted language (R, Python, Matlab, etc.) to perform analysis or make corrections and save that information in a separate file.]
...
If a script changes the content of a raw data file, .gradient-underline[chances are that the same script will not work again the second time you run it because the structure of the file has changed].

<center>
  <figure>
      <img src="img/keep_data_raw.png" width = "800" alt="Drawning showing that data can change after the data is cleaned."/>
  </figure>
</center>
...
<center>
  <figure>
      <img src="img/project_structure.png" height = "500" alt="Image showing how I do organize my files in a data project."/>
  </figure>
</center>
...
- .gradient-underline[A common problem is that the columns represent values, not variable names.]
  - **Often occurs with datasheets containing species abundance.**

<center>
  <figure>
      <img src="img/table_species.png" width = "500" alt="Image showing a frequent method to enter count data in spreadsheet softwares."/>
  </figure>
</center>
...
As an example, we will use a dataset where plant biomasses of **three species** were measured under **two experimental treatments** in the Arctic Tundra.
...
<div><img src="img/plants/dryas_integrifolia.jpg" height="200"/>
<figcaption>Dryas Integrifolia</figcaption></div>
<div><img class="middle-img" src="img/plants/festuca_brachyphylla.jpg" height="200"/><figcaption>Festuca Brachyphylla</figcaption></div>
<div><img src="img/plants/luzula_nivalis.jpg" height="200"/><figcaption>Luzula Nivalis</figcaption></div>

</div>
</center>

<small>
Images from [Arctic Flora of Canada and Alaska](https://arcticplants.myspecies.info/)
</small>
...
One common mistake is that information (such as **species**) are contained within column names. Note the **spaces** and **special characters** (/, $\pm$ and []) in the column names.
...
After proper transformations, the data is now tidy ([or in normal form](https://en.wikipedia.org/wiki/Database_normalization)). 
...
df_pivoted |>
  filter(!str_detect(vegetation_type, "Sedge")) |>
...
If you use a spreadsheet program, **keep your data arranged as rectangular tables**. Otherwise, .gradient-underline[it makes data importation difficult]. 
...
- Use an incremental strategy to backup your data (.gradient-underline[ideally daily]).
...
<img src="img/unsplash/wilhelm-gunkel-AKQlYooS72w-unsplash.jpg" class="centerImage" height="500" alt="Paper sheet in a old typewriter with thank you written in different language.">
