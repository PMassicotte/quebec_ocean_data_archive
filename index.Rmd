...
date: "October 18, 22 and 25, 2021"
output:
...
March 22, 2021 (updated: `r Sys.Date()`)
...
`r fontawesome::fa("times", fill = "#E6352FFF")` Extra libraries are needed (*libnetcdf-dev, libnetcdfc++4*): can not be read as easy as plain text files
...
An example of GeoTIFF image of the Europe. <small>Data: https://bit.ly/2M8jAs1</small>
  
```{r geotiff, cache=TRUE, fig.height=3, fig.asp=NA, dev='png', dpi=300}
...
<center>
  ...
  <small>Image from https://r4ds.had.co.nz</small>
...
File naming can be a nightmare for the future you and your colleagues.
...
Always use the ISO format: <big>**yyyy**</big>-<medium>**mm**</medium>-<small>**dd**</small> (large `r fontawesome::fa("arrow-right", fill = "#d5695d")` small)
...
ggplot() +
  geom_sf(data = wm, fill = NA, size = 0.1) +
  geom_sf(data = usa, fill = "red", size = 0.1) +
  labs(
    title = str_wrap("Comprehensive map of every country that use MMDDYYYY date format", 50)
  ) +
  coord_sf(crs = 8857)
```
...
1. Do not make any changes/corrections to the original raw data file.

--
  
2. Use a scripted language to perform analysis or make corrections and save that information in a separate file.

--
  
3. If you want to do some analyses in Excel, make a copy of the file and do your calculations and graphs in the copy.
...
- Why we want tidy data?

- Well-formatted data allows for quick visualization, modeling, manipulation .gradient-underline[and archiving].
...
To make this data **tidy**, we have to *pivot* it longer. Two new columns (`year` and `number_of_cases`) are created.
...
```{r plot_temperature, out.width="85%", cache=TRUE}
df %>%
...
`r fontawesome::fa("exclamation-triangle", fill = "#ffb82a")` .gradient-underline[Do not use a numerical (ex.: **999**) value to indicate missing values.]
...
- Use an incremental strategy to backup your data (daily).
...
- Git is a version control system (Wikipedia): 
